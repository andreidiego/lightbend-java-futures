...
class CustomerService {
    ...
    CompletableFuture<Optional<String>> getCustomerFirstName(UUID customerId) {
        return customerRepository.getCustomer(customerId).thenApply(optionalCustomer -> optionalCustomer.map(Customer::getFirstName));
    }

    CompletableFuture<Optional<String>> getCustomerLastName(UUID customerId) {
        return customerRepository.getCustomer(customerId).thenApply(optionalCustomer -> optionalCustomer.map(Customer::getLastName));
    }

    CompletableFuture<Optional<String>> getCustomerAddress(UUID customerId) {
        return customerRepository.getCustomer(customerId).thenApply(optionalCustomer -> optionalCustomer.map(Customer::getAddress));
    }

    CompletableFuture<Optional<String>> getCustomerPhoneNumber(UUID customerId) {
        return customerRepository.getCustomer(customerId).thenApply(optionalCustomer -> optionalCustomer.map(Customer::getPhoneNumber));
    }
}
